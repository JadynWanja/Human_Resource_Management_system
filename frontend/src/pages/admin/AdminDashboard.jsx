  ...
  return (
    <main className='admin-layout' data-node-id='2:42'>
      <aside className='sidebar' aria-label='Admin navigation'>
        <div>
          <p className='sidebar-logo'>HRMS</p>
          <p className='sidebar-subtitle'>Human Resource Management</p>
        </div>
        ...
        <section className='panel requests-panel'>
          ...
          <div className='request-list'>
            {leaveRequests.map((request) => (
              <div className='request-row' key={request.name + request.type}>
                <span>
                  {request.name} - {request.type} - {request.status}
                </span>
                <strong className={'status ' + request.status.toLowerCase()}>
                  {request.status}
                </strong>
              </div>
            ))}
